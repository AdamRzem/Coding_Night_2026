<script lang="ts">
	...
	let activeNavLinks = $derived(
		isEmployeeSection ? workerNavLinks : isClientSection ? clientNavLinks : publicNavLinks
	);

	let brandName = $derived(
		isEmployeeSection ? 'Staff Panel' : isClientSection ? 'My Canteen' : 'Login'
	);
	...
	let brandHref = $derived(isEmployeeSection ? '/employee' : isClientSection ? '/client' : '/');
...
<div
	class="min-h-screen bg-gray-50 text-gray-900 transition-colors duration-300 dark:bg-gray-950 dark:text-gray-100"
>
	<nav
		class="sticky top-0 z-50 border-b border-gray-200 bg-white/80 backdrop-blur-lg dark:border-gray-800 dark:bg-gray-900/80"
	>
		<div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
			<div class="flex h-16 items-center justify-between">
				<a
					href={brandHref}
					class="flex items-center gap-2 text-xl font-bold {brandClasses} transition-colors"
				>
				...
				<div class="flex items-center gap-1">
					...
					<div
						class="ml-2 flex items-center gap-2 border-l border-gray-200 pl-2 dark:border-gray-700"
					>
						{#if data.user}
							<span
								class="hidden max-w-32 truncate text-sm text-gray-600 sm:inline dark:text-gray-400"
							>
								{displayName}
							</span>
							<button
								onclick={handleSignOut}
								class="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition-all duration-200 hover:bg-red-50 hover:text-red-600 dark:text-gray-400 dark:hover:bg-red-900/20 dark:hover:text-red-400"
							>
								<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path
										stroke-linecap="round"
						...
						{:else}
							<a
								href="/login"
								class="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium {signInBtnClasses} text-white shadow-sm transition-all duration-200 hover:shadow"
							>
								<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path
										stroke-linecap="round"
	...
	{#if isOffline}
		<div
			class="border-b border-amber-300 bg-amber-50 text-amber-900 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200"
		>
			<div class="mx-auto max-w-5xl px-4 py-2 text-sm font-medium sm:px-6 lg:px-8">
				No internet connection. Some data and actions may fail until you are back online.
			</div>
		</div>
	{/if}
	...
	<footer class="mt-12 border-t border-gray-200 dark:border-gray-800">
		<div
			class="mx-auto max-w-5xl px-4 py-6 text-center text-sm text-gray-400 sm:px-6 lg:px-8 dark:text-gray-600"
		>
